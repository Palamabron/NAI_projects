...
from knapsackProblem import txtToKnapsack, knapsack
...
def menu():
    ...
    window = sg.Window('Knapsack problem', createPathMenu())
    ...
    while True:
        ...
        if isPathMenu:
            if event == 'pathEnter':
                ...
                if dataPath is None or dataPath == "":
                    ...
                    if exists(dataPath):
                        data = txtToKnapsack(path=dataPath)
                    else:
        ...
        if isUserMenu:
            if event == 'bruteForce':
                knapsack(data[0][0], data[1], data[2])
